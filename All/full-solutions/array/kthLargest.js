...
// heap has O(nlogn) time complexity and O(n) space complexity  where as quick select has O(n) time complexity and O(1) space complexity when k is less than n/2 else O(n)*O(n) = O(n^2) time complexity and O(1) space complexity ?
//correct ?  O(n) space complexity for quick select is wrong ?